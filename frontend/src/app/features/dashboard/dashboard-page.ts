...
export class DashboardPage implements OnInit {
  ...
  load(): void {
    ...
    this.api.dashboard().subscribe({
      ...
      error: () => {
        this.error.set('Check that the API is running, then try again.');
        this.loading.set(false);
      },
    });
  }
  ...
  completion(stats: DashboardSummary): number {
    const total = stats.openTaskCount + stats.completedTaskCount;
    return total ? Math.round((stats.completedTaskCount / total) * 100) : 0;
  }
    ...
    return [
      {
        ...
        note: `${stats.activeProjectCount} currently active`,
        icon: 'folder_open',
        tone: 'blue',
      },
      {
      ...
      },
      {
        ...
        note: `${stats.dueSoonCount} due this week`,
        icon: 'checklist',
        tone: 'amber',
      },
      {
